...
def exercise08():
    ...
    # Calculate the average square feet
    avg_sq_ft = df['sq__ft'].mean()

    # Filter transactions in zip code 95670
    df_zip_95670 = df[df['zip'] == 95670]

    # Filter transactions not in zip code 95610
...
def exercise11(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    # Create a 1D array of integers 0 to n-1
    array_1d = np.arange(n)

    # Reshape the array into 3 rows and n/3 columns
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    date_range = pd.date_range(start = '2010-01-01', periods=n)

    # Generate n random integers between 0 and n
    s = pd.Series(np.random.randint(0, n+1, size=n), index=date_range)
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(words).map(lambda x: len(x))
...
def exercise15():
    ...
    # Load the real estate transaction data into a DataFrame (assuming it's stored in a CSV file)
    real_estate_data = pd.read_csv('https://tinyurl.com/y63q7okz')

    # Extract every 5th row using iloc and select only the "street address" and "zip code" columns
    df = real_estate_data.iloc[::5, [0, 4]]
